...
function PureKnob() {
	 ...
	 */
	this.createKnob = function(width, height, fontSizeModifier) {
		var heightString = Math.round(height / 2.0).toString();
		var widthString = Math.round(width / 2.0).toString();
		...
		var knob = {
			...
			'_timeout': null,
			'_width': width,
            '_fontSizeModifier': fontSizeModifier,
			...
			'redraw': function() {
                ...
                if (excess > 0) {
                    ctx.beginPath();
					ctx.arc(centerX, centerY, radius, angleVal, angleVal + excessVal);
                    ctx.lineCap = 'butt';
                    ctx.lineWidth = lineWidth;
                    ctx.strokeStyle = colorExcess;
                    ctx.stroke();
					...
					if (excessLabel != '') {
						...
						ctx.fillText(excessLabel, targX, targY);
					}
                }
                ...
                if (needle) {
                    ctx.beginPath();
                    var origX = centerX + 0.2 * radius * Math.cos(angleVal);
                    var origY = centerY + 0.2 * radius * Math.sin(angleVal);
                    var targX = centerX + (1.0 - 1.1 * trackWidth / 2.0) * radius * Math.cos(angleVal);
                    var targY = centerY + (1.0 - 1.1 * trackWidth / 2.0) * radius * Math.sin(angleVal);
				...
				ctx.textAlign = 'center';
				ctx.textBaseline = 'middle';
				ctx.fillText(prefix + valueStr + suffix, centerX, centerY);
				
				/*
				 * Draw the range.
				 */
                if (showRange) {
                    ctx.fillText(prefix + valMinStr + suffix,
                        centerX - radius, centerY);
                    ctx.fillText(prefix + valMaxStr + suffix,
                        centerX + radius, centerY);
                }
